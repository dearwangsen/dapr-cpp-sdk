...
{
public:
  EchoApp(std::string dapr_grpc_port, std::string app_port) : dapr_grpc_port_(dapr_grpc_port), app_port_(app_port) {}

  void ConnectToDapr()
  {
    // 连接到 dapr grpc 服务器
    std::cout << "Connecting to " << std::string("127.0.0.1:") + dapr_grpc_port_ << std::endl;
    client_stub_ = dapr::proto::runtime::v1::Dapr::NewStub(grpc::CreateChannel(std::string("127.0.0.1:") + dapr_grpc_port_, grpc::InsecureChannelCredentials()));
  }

  std::string CallMethod(const std::string app_id, const std::string method, const std::string message)
  {
    grpc::ClientContext context;                            // grpc上下文
    dapr::proto::runtime::v1::InvokeServiceRequest request; // InvokeService 请求
    ...
    request.mutable_message()->mutable_data()->set_value(message.c_str()); //"hello dapr"
    // 调用 InvokeService
    grpc::Status status = client_stub_->InvokeService(&context, request, &response);
    if (status.ok())
    {
...
private:
  std::string app_port_;                                              // 6100端口，用于dapr客户端访问，需要与dapr启动时的端口保持一致
  std::string dapr_grpc_port_;                                        // dapr端口号
  std::unique_ptr<dapr::proto::runtime::v1::Dapr::Stub> client_stub_; // dapr 的存根
...
int main(int argc, char **argv)
{
  ...
  if (grpc_port == "")
    grpc_port = "50001";
  std::cout << "grpc port:" << grpc_port << " app port:" << app_port << std::endl;
